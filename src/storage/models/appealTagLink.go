...
type AppealTagLink struct {
	...
	AppealId uuid.UUID `json:"-" gorm:"column:appeal_id;type:uuid;not null"`
	Appeal   Appeal    `json:"-" gorm:"foreignKey:appeal_id;references:id"`
	TagId    uuid.UUID `json:"-" gorm:"column:tag_id;type:uuid;not null"`
	Tag      Tag       `json:"-" gorm:"foreignKey:tag_id;references:id"`
